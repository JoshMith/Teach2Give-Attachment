...
const fetchBooks = async () =>  {
    try {
        const response:AxiosResponse<Book[]>  = await axios.get(`${API_URL}/v1/books`);
        // const booksData = await response.json(); // Convert response to JSON
...
//     try {
//         const response: Response = await fetch("http://localhost:3000/api/v1/books");
...
export function displayBooks(filteredBooks: Book[]): void {
